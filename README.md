...
**onPanda**(**on**-**P**olicy **a**lig**n**ment **d**ata **a**nnotator) is a data annotation tool, or solution, for LLM alignment data(both SFT and RL). By innovatively combining the characteristics of front-end engineering and GPT models, annotators can efficiently generate SFT responses with the model's aid, while also producing on-policy token-level preference data.
