...
4. 蓝色小圆点表示对应对话的答复是新生成的，而不是基于续写的`,
    ...
    stream: '部分 API 供应商对流式（stream）模式的支持不够完善，可能导致续写或 top_logprobs 功能出现异常。遇到这类问题时，可以尝试关闭 stream。',
...
- 双击修改：可以双击词组对其修改，随后模型会基于修改后的内容续写回复
...
**上手建议：**
- 可随意尝试任何按钮，按钮大都自带提示说明
- 建议依次点击下方的所有示例来感受 onPanda 的用法

**标注要求**
- 若您使用 onPanda 不是为了标注数据，可忽律这一部分
- 角色信息：system 是系统指令，user 是用户消息，assistant 是模型回复
- 尽可能用“候选续写”，如果候选词组中没有合适的词组，再考虑“双击修改”问题词组
- 请删掉非标注相关的历史对话后再保存

**进阶功能**
- 选中改写：可以选中一段文本进行改写，改写后的文字会有蓝色背景标记
- 候选词组：
  - 在候选词组上点击鼠标 \`右键\` 或 \`Ctrl + 点击\`，将用候选词组替换当前词组
  - 在候选词组上点击鼠标 \`中键\` 或 \`Alt + 点击\`，会复制候选词组到剪贴板
`,
  },
  selectedTextPanel: {
